...
- **`ProjectsSidebar`** — reads registry on mount; refreshes sync status (and drift counts) every 30 s; right-click context menu (Reveal in Finder / Sync now / Unlink); "Add project" triggers `pick_directory`. Footer contains "Backlog" toggle and `SettingsTrigger`. Each project row shows a clickable amber dot (`bg-amber-400`) when `drift_count > 0`; clicking the dot calls `openDriftPanel(project.name)` and opens the `DriftPanel` dialog. The dot is a `<span role="button">` (rather than `<button>`) because it lives inside the project-row `<button>` element.
