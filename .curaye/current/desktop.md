...
| `usePaletteStore` | AI palette phase (input/streaming/diff), query, resolved action, streamed text, diff lines, AI config cache. Before streaming, `execute()` runs a `gatherRagContext` pre-flight for the three enriched commands (draft-spec, generate-ac, semantic-search): loads prd.md + stack.md via `generate_brief_context`, runs keyword or semantic search scoped to the relevant doc sections, reads full document bodies, and budgets context to 12 000 chars total. Result is stored on `PaletteContext.ragContext: RagContext \| null` and injected into the system message. `AbortController` is created before the pre-flight so cancellation covers both phases. |
...
- **`AIPalette`** — `⌘K` modal with three phases: (1) input: free-text query + suggestion chips; (2) streaming: token-by-token output with blinking cursor, Cancel button; (3) diff: side-by-side Before/After view with Apply / Edit first / Discard actions. Does not open if a text input has focus. Restores focus on close. Shows an "AI not configured" view when no provider is set. For "Draft a spec", "Generate acceptance criteria", and "Find where I solved" queries, a RAG pre-flight runs before the AI call: searches relevant doc sections (planned/shipped for drafting; shipped/planned for AC; all four for find-where-solved), reads full document bodies, and injects prd.md + stack.md as standing project context into the system message (12 000-char budget total). Escape/Cancel aborts both the pre-flight and any in-flight stream.
