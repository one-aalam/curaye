...
import { AlertCircle, AlertTriangle, ChevronDown, ChevronRight, Pencil, Eye } from "lucide-react";
...
function MetadataStrip({ onEdit }: { onEdit: () => void }) {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 border-b border-border/30 shrink-0 min-h-7.5">
      {status && (
        <span
          className={cn(
      ...
      {effort && (
        <span className="text-[10px] font-mono text-muted-foreground/60 tabular-nums uppercase font-black">{effort}</span>
      )}
      {impact && (
        <span className="text-[10px] text-muted-foreground/50">
          <span className="text-muted-foreground/25 mr-0.5 uppercase">impact:</span>{impact}
        </span>
      )}
      {desire && (
        <span className="text-[10px] text-muted-foreground/50">
          <span className="text-muted-foreground/25 mr-0.5 uppercase">desire:</span>{desire}
        </span>
      )}
  ...
  return (
    <div
      className={cn(
        ...
        "bg-black/60 border-l border-(--glass-border)",
        "transition-transform duration-200 ease-out",
        open ? "translate-x-0" : "translate-x-full",
      )}
    >
      <div className="flex items-center justify-between px-4 py-3 backdrop-blur-(--glass-blur) shadow-(--glass-shadow) border-b border-(--glass-border) shrink-0">
